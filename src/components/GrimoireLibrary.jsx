...
export default function GrimoireLibrary({ projects, enabled, github }) {
  ...
  const pages = grouped[active?.id] || []
  function chooseShelf(id) { setActiveShelf(id) }
  ...
  return <section className="orbital-library" aria-label="Biblioteca orbital de proyectos">
    ...
    <div className="library-filters" role="tablist" aria-label="Filtrar proyectos por área">{available.map(shelf => <button key={shelf.id} role="tab" aria-selected={activeShelf === shelf.id} onClick={() => chooseShelf(shelf.id)}><span>{shelf.sigil}</span>{shelf.title}<small>{grouped[shelf.id].length}</small></button>)}</div>
    <div className="library-results" aria-live="polite"><p className="result-label">{active.title} · {pages.length} proyectos</p><AnimatePresence mode="wait" initial={false}><motion.div className="project-library-grid" key={active.id} initial={enabled ? { opacity: 0, y: 14 } : false} animate={{ opacity: 1, y: 0 }} exit={enabled ? { opacity: 0, y: -10 } : undefined} transition={{ duration: .32, ease: [.2,.65,.3,1] }}>{pages.map((project, index) => <motion.article className="library-card" key={project.repo} initial={enabled ? { opacity: 0, y: 16 } : false} animate={{ opacity: 1, y: 0 }} transition={{ duration: .32, delay: Math.min(index * .05, .25) }} whileHover={enabled ? { y: -5 } : undefined}><p className="page-mark">{String(index + 1).padStart(2, '0')} / {project.category}</p><h4>{project.title}</h4><p>{project.description}</p><div className="tags">{project.stack.map(tag => <span key={tag}>{tag}</span>)}</div><a className="page-link" href={project.url || `${github}/${project.repo}`} target="_blank" rel="noreferrer">Abrir repositorio <ArrowUpRight size={18}/></a></motion.article>)}</motion.div></AnimatePresence></div>
  </section>
}
