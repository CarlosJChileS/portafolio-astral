...
export default function App() {
  ...
  return <>
    ...
    <motion.div className="reading-progress" style={{ scaleX: scrollYProgress }} aria-hidden="true"/><main id="contenido" className="portfolio-main"><section ref={heroRef} className="hero" id="inicio" onPointerMove={followPointer} onPointerLeave={resetPointer}>
      ...
      <div className="hero-copy shell"><p className="serif-intro">Carlos Chile Silva <span>desarrollador de IA desde Ecuador.</span></p><h1><span className="title-mask"><span className="title-line">IA que conecta</span></span><span className="title-mask title-second"><span className="title-line">decisiones<StarMark className="title-star"/></span></span></h1><div className="hero-signature"><AquariusConstellation/><span>{identity.sign} / {identity.birthDate}</span><small>{identity.constellationLabel}</small></div><div className="hero-bottom"><p>Agentes, datos y experiencias full stack para convertir señales complejas en decisiones claras.</p><a className="round-link" href="#proyectos" aria-label="Explorar proyectos"><ArrowDown size={23}/></a><div className="hero-controls"><span className="edition">PORTAFOLIO ASTRAL<br/>PROYECTOS · SISTEMAS · CÓDIGO</span><button className="motion-toggle" onClick={()=>setPaused(!paused)} aria-pressed={paused || !!reduced} disabled={!!reduced}>{paused || reduced ? <Play size={12}/> : <Pause size={12}/>}<span>{reduced ? "Movimiento reducido" : paused ? "Activar movimiento" : "Pausar movimiento"}</span></button></div></div></div>
    </section>
